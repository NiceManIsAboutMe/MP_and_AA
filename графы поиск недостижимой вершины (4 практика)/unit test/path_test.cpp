...
TEST_CASE( "[Path] Empty graph", "[path_exists]" ) {
    Graph g;    
    CHECK_FALSE( path_exists(g, 0, 1) );
}

TEST_CASE( "[Path] Singleton graph", "[path_exists]" ) {
    Graph g;    
    g.add_vertex(0);
    CHECK( path_exists(g, 0, 0) );
}

TEST_CASE( "[Path] Singleton graph with a loop", "[path_exists]" ) {
    Graph g;    
    g.add_vertex(0);
    g.add_arc(0, 0);
    CHECK( path_exists(g, 0, 0) );
...
TEST_CASE( "[Path] Path for two vertices without an arc", "[path_exists]" ) {
    Graph g;    
    g.add_vertex(0);
    g.add_vertex(1);
    CHECK_FALSE( path_exists(g, 0, 1) );
...
TEST_CASE( "[Path] Path for two vertices with an arc", "[path_exists]" ) {
    Graph g;    
    g.add_arc(0, 1);
...
TEST_CASE( "[Path] Path for several vertices", "[path_exists]" ) {
    Graph g;    
    g.add_arc(0, 1);
    g.add_arc(1, 2);
    g.add_arc(2, 3);
...
TEST_CASE( "[Path] Path in a disconnected graph", "[path_exists]" ) {
    Graph g;    
    g.add_arc(0, 1);
    g.add_arc(1, 2);
    g.add_arc(2, 3);
...
TEST_CASE( "[Path] Path with non-existing vertices", "[path_exists]" ) {
    Graph g;    
    g.add_arc(0, 1);
    g.add_arc(1, 2);
    g.add_arc(2, 3);
...
TEST_CASE( "[Path] Path in a graph with loops", "[path_exists]" ) {
    Graph g;    
    g.add_arc(0, 1);
    g.add_arc(1, 2);
    g.add_arc(2, 3);
...
TEST_CASE( "[Path] Graph with negative vertices", "[path_exists]" ) {
    Graph g;    
    g.add_arc(-1, -999);
    g.add_arc(-1, -2);
    g.add_arc(-2, 0);
...
TEST_CASE( "[Path] Graph with many loops", "[path_exists]" ) {
    Graph g;    
    g.add_arc(0, 1);
    g.add_arc(1, 2);
    g.add_arc(1, 3);
